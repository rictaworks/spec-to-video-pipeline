...
| `references/remotion-project.md` | P2（7章のみ）・P7・P11・P12 |
...
| P2 | 前提ゲート | 環境・ライセンス・資格情報・上限・実行機の資源・出力先が確定していること | 判断（`cost-and-credentials.md`・`remotion-project.md` 7章） |
...
**尋ねる前に、設計書・絵コンテ・企画書を読みます。** 資料に定めがある事項を尋ねると二重確認になります。読んだうえで定めが無い場合にかぎり尋ねます。

## 出力先の確定（P2）

成果物を書き出す場所は**案件ごとに違います。** 本ファイルの「出力」節が挙げる置き場は既定の候補であって、案件の取り決めではありません。

確定は次の順で行います。

```mermaid
graph TD
    A[P2 出力先の確定] --> B{設計書・絵コンテ・企画書に<br>出力先の定めがありますか}
    B -- はい --> C[その定めに従います<br>尋ねません]
    B -- いいえ --> D[選択肢を添えて尋ねます<br>既定の置き場を第1候補にします]
    C --> E[どこに定めがあったかを<br>実行レポートへ記録します]
    D --> F[回答と根拠を<br>実行レポートへ記録します]
```

1. **先に資料を読みます。** 設計書・絵コンテ・企画書に、リポジトリ構成や成果物の置き場の定めがあるかを確認します
2. 定めがあれば**それに従います。尋ねません**
3. 定めが無い場合にかぎり、選択肢を添えて尋ねます。既定の置き場を第1候補として示します

確定した出力先と、**設計書のどこに定めがあったか**（または尋ねた結果）を実行レポートへ記録します。

**出力先が確定していない状態で P12 へ進みません。** 書き出してから置き場を決めると、成果物の所在が資料と食い違います。
...
| シーン境界で輝度が沈む | 前後を同時に半透明にしている | P11（重ね方の修正） | レンダのみ |
...
置き場は**既定の候補**です。案件の資料に定めがある場合はそちらに従います（「出力先の確定（P2）」を参照）。
